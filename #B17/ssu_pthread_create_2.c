...
int main(void)
{
	pthread_t tid[THREAD_NUM]; //쓰레드 여러개 만들 것을 알 수 있음
	int sum;
	int i;

	sum = 0;
	messages[0] = "Englist: Hello World!";
	...
	for(i=0 ; i < THREAD_NUM ; i++){
		...
		if(pthread_create(&tid[i], NULL, ssu_printhello, (void*)&thread_data_array[i]) != 0){ //쓰레드 THREAD_NUM 개수만큼 생성
...
void *ssu_printhello(void *arg){
	...
	printf("Thread %d: %s Sum=%d\n", thread_index, message, sum); //구조체의 thread_index, message, sum 출력
	return NULL;
}
